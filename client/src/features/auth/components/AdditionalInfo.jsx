...
const AdditionalInfo = ({ role, subRole, onBack, onSkip, onComplete }) => {
  ...
  const handleCategoriesChange = (newCategories, uploadData = null) => {
    if (uploadData) {
      const { categoryId, requirementTitle, file } = uploadData;
      if (requirementTitle === "multiple") {
        setFormData((prev) => ({
          ...prev,
          categoryUploads: {
        ...
        setFormData((prev) => ({
          ...
          categoryUploads: {
            ...prev.categoryUploads,
            [categoryId]: {
              ...prev.categoryUploads[categoryId],
              [requirementTitle]: file,
            },
          },
        }));
      }
    } else {
      if (newCategories !== formData.categories) {
    ...
    <div className="w-full max-w-[560px] text-left px-4 sm:px-0">
      ...
      <div className="flex items-center gap-2 mb-6">
        <div
          className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
            currentSubStep >= 1 ? "bg-[#024CEE] border-[#024CEE] text-white" : "border-[#E5E7EB] text-[#6B7280]"
          }`}
        >
          1
        </div>
        <div className={`flex-1 h-0.5 rounded ${currentSubStep >= 2 ? "bg-[#49B7DF]" : "bg-[#E5E7EB]"}`} />
        <div
          className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
            currentSubStep >= 2 ? "bg-[#024CEE] border-[#024CEE] text-white" : "border-[#E5E7EB] text-[#6B7280]"
          }`}
        >
          2
        </div>
        <div className={`flex-1 h-0.5 rounded ${currentSubStep >= 3 ? "bg-[#49B7DF]" : "bg-[#E5E7EB]"}`} />
        <div
          className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
            currentSubStep >= 3 ? "bg-[#024CEE] border-[#024CEE] text-white" : "border-[#E5E7EB] text-[#6B7280]"
          }`}
        >
          3
        </div>
      ...
      {currentSubStep === 1 && (
        <div className="fade-in">
          <div className="text-[1.375rem] font-bold text-[#111827] mb-1">Profile Setup</div>
          <div className="text-sm text-[#6B7280] mb-5">
            Add a photo and a short bio so clients know who they're working with.
          </div>

          <div className="mb-5">
            <label className="block text-sm font-medium text-[#111827] mb-2">
              Profile Image / Logo{" "}
              <span className="text-[#6B7280] font-normal text-xs">— Square 1:1, high resolution</span>
            </label>
            <ProfileImageUpload
          ...
          <div className="mb-5">
            <label className="block text-sm font-medium text-[#111827] mb-2">
              Bio <span className="text-[#6B7280] font-normal text-xs">— 80 to 600 characters</span>
            </label>
            <textarea
              value={formData.bio}
            ...
            <div className="flex justify-between mt-1 ml-1">
              <span className="text-[11px] text-[#6B7280]">
                {formData.bio.length < 80 ? "Minimum 80 characters recommended" : ""}
              </span>
              <span className="text-xs text-[#9CA3AF]">{formData.bio.length}/600</span>
            </div>
          </div>
      ...
      {currentSubStep === 2 && (
        <div className="fade-in">
          <div className="text-[1.375rem] font-bold text-[#111827] mb-1">Your Category</div>
          <div className="text-sm text-[#6B7280] mb-5">
            Select the service(s) you offer. Click on a category to see requirements and upload credentials.
          </div>
      ...
      {currentSubStep === 3 && (
        <div className="fade-in">
          <div className="text-[1.375rem] font-bold text-[#111827] mb-1">Identity Verification</div>
          <div className="text-sm text-[#6B7280] mb-5">
            Upload your ID to activate your provider account. Documents are reviewed securely by our team and never shared publicly.
          </div>
          ...
          <div className="mb-5">
            ...
            <p className="text-sm text-[#6B7280] mb-4 leading-relaxed">
              Upload your credentials to earn badges that appear on your public profile. The more you verify, the higher you rank in search results.
            </p>

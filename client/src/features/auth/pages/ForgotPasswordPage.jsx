...
const ForgotPasswordPage = () => {
  ...
  const handleResendEmail = async () => {
    ...
    try {
      await forgotPassword(email);
      setSuccessMsg("New reset link sent! Check your inbox.");
      startTimer();
    } catch (err) {
          ...
          <div className="w-full max-w-[420px]">
                ...
                <div className="flex items-center gap-0 mb-7">
                  {[1, 2].map((s) => (
                    <React.Fragment key={s}>
                      ...
                      >
                        {s < step || (step === 2 && resetToken) ? "✓" : s}
                      </div>
                      {s < 2 && (
                        <div className={`flex-1 h-0.5 transition-all ${s < step || (step === 2 && resetToken) ? "bg-green-500" : "bg-[#E5E7EB]"}`} />
                      )}
                    </React.Fragment>
                  ))}
                </div>
                  ...
                  {["Email", "New Password"].map((label, i) => (
                    <span
                      ...
                      className={`text-[11px] font-medium text-center flex-1 ${
                        i + 1 === step || (step === 2 && resetToken && i === 1)
                          ? "text-[#024CEE] font-bold"
                          : i + 1 < step || (step === 2 && resetToken && i === 1)
                          ? "text-green-500"
                          : "text-[#6B7280]"
                      }`}
            ...
            {step === 1 && (
              <div className="animate-fade-up">
                <div className="text-xs font-semibold text-[#024CEE] uppercase tracking-wider mb-2">Step 1 of 2</div>
                <h2 className="text-2xl font-extrabold mb-1.5">Forgot your password?</h2>
                <p className="text-sm text-[#6B7280] mb-5">Enter your email and we'll send you a reset link.</p>
            ...
            {step === 2 && !resetToken && (
              <div className="animate-fade-up">
                <div className="text-xs font-semibold text-[#024CEE] uppercase tracking-wider mb-2">Step 2 of 2</div>
                <h2 className="text-2xl font-extrabold mb-1.5">Check your email</h2>
                <p className="text-sm text-[#6B7280] mb-5">
                  We sent a reset link to <b className="text-[#111827]">{maskEmail(email)}</b>
                </p>
                ...
                <div className="p-4 rounded-xl bg-[#F8FAFF] border border-[#E0E7FF] mb-4">
                  <p className="text-sm text-[#111827]">
                    Click the reset link in your email to continue. The link expires in 1 hour.
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  >
                    {timer > 0 ? `Resend (${Math.floor(timer / 60)}:${(timer % 60).toString().padStart(2, "0")})` : "Resend link"}
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-[#F3F4F6] text-[#374151] font-bold text-sm cursor-pointer transition-all hover:bg-[#E5E7EB] border border-[#E5E7EB]"
                >
